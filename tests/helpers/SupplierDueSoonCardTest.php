 ...
 *
 *   - overdue  : payment.Deadline  <  today   (already passed)
 *   - today    : payment.Deadline  =  today
 *   - tomorrow : payment.Deadline  =  today+1
 *
 * All three buckets share the same row filters; only the Deadline
 * predicate differs. A supplier payout row is in scope when:
 *
 ...
 *   - payment.Deadline <= today+1                   (window = overdue..tomorrow)
 *
...
$bucketStmt = $pdo->prepare("
    ...
    FROM payment
    WHERE {$base}
      AND payment.Deadline <= :tmr2
");
$bucketStmt->execute(array(':today' => $today, ':tmr' => $tomorrow, ':tmr2' => $tomorrow));
...
// ---- Bucket headlines --------------------------------------------------
assert_eq('overdue count (Alpha minus1 + Beta minus2)', 2,      (int)   $b['overdue_cnt']);
assert_eq('overdue total (500+300)',                    800.0,  (float) $b['overdue_due']);
...
assert_eq('Alpha due_count (minus1+today+tmr)',  3,         (int)   $by_sid[1]['due_count']);
...
assert_eq('Beta due_count (minus2+tmr)',         2,         (int)   $by_sid[2]['due_count']);
assert_eq('Beta due_total (300+400)',            700.0,     (float) $by_sid[2]['due_total']);
assert_eq('Beta earliest_deadline',              $minus2,   (string)$by_sid[2]['earliest_deadline']);

// Earliest-deadline ASC — Beta (minus2) is more overdue, so it leads.
...
assert_eq('buckets partition: total amount', 2100.0,   $bucket_total);
