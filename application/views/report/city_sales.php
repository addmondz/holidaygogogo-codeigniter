    ...
    <div class="container-fluid">
        <div class="card card-custom mb-5">
            ...
            <div class="card-body">
                <div class="accordion accordion-solid accordion-toggle-plus">
                    <div class="card">
                        <div class="card-header">
                            <div id="city_sale_header" data-toggle="collapse" data-target="#city_sale_info" class="card-title collapsed" style="font-size:13px;">Filter By Category / Booking Information</div>
                        </div>
                        <div id="city_sale_info" class="collapse">
                            <div class="card-body">
                                <form id="form" action="<?php echo base_url('Report/City_Sales') ?>" method="get" class="form">
                                    ...
                                    <div class="row">
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                <label>City</label>
                                                <select name="city" data-live-search="true" class="form-control selectpicker">
                                                    <option selected data-icon="la la-city font-size-lg bs-icon" value="">--SELECT CITY--</option>
                                                    <?php foreach($cities as $city) { ?>
                                                        <option data-icon="la la-city font-size-lg bs-icon" value="<?php echo $city->City; ?>" <?php if(!empty($this->input->get('city')) && $this->input->get('city') == $city->City) { echo 'selected'; } ?>><?php echo $city->City; ?></option>
                                                    <?php } ?>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                <label>State</label>
                                                <select name="state" data-live-search="true" class="form-control selectpicker">
                                                    <option selected data-icon="la la-city font-size-lg bs-icon" value="">--SELECT STATE--</option>
                                                    <?php foreach($states as $state) { ?>
                                                        <option data-icon="la la-city font-size-lg bs-icon" value="<?php echo $state->State; ?>" <?php if(!empty($this->input->get('state')) && $this->input->get('state') == $state->State) { echo 'selected'; } ?>><?php echo $state->State; ?></option>
                                                    <?php } ?>
                                                </select>
                                            </div>
                                        </div>
                ...
                <br><br>
                <div class="dataTables_wrapper dt-bootstrap4 no-footer" <?php if(empty($city_sales)) { echo 'style="overflow-x:auto;"'; } ?>>
                    <table id="kt_datatable" class="table table-bordered table-head-custom table-checkable dataTable no-footer dtr-inline">
                        ...
                        <tbody>
                            <?php if(empty($city_sales)) { ?>
                                <td colspan="6" style="text-align:center; padding-top:10px; padding-bottom:10px;">City Sales Not Found</td>
                            <?php } else { ?>
                                <?php $count = 1; ?>
                                <?php foreach($city_sales as $city_sale) { ?>
                                    <tr>
                                        <td style="text-align:center; padding-top:15px; padding-bottom:15px;"><?php echo $count; ?></td>
                                        <td style="text-align:center;"><?php echo $city_sale->Month; ?></td>
                                        <td style="text-align:center;"><?php echo $city_sale->City; ?></td>
                                        <td style="text-align:center;"><?php echo $city_sale->NetTotal; ?></td>
                                        <td style="color:<?php if($city_sale->Profit < 0) { echo '#FF2400;'; } else if($city_sale->Profit == 0) { echo '#F4BB44;'; } else { echo '#00A36C;'; } ?> text-align:center;"><?php echo $city_sale->Profit; ?></td>
                                        <td style="color:<?php if($city_sale->Profit < 0) { echo '#FF2400;'; } else if($city_sale->Profit == 0) { echo '#F4BB44;'; } else { echo '#00A36C;'; } ?> text-align:center;"><?php echo $city_sale->Margin; ?></td>
                                    </tr>
                                    <?php $count++; ?>
                                <?php } ?>
                            <?php } ?>
...
<script>
    ...
    <?php if(!empty($this->input->get('city')) || !empty($this->input->get('state')) || !empty($this->input->get('country')) || !empty($this->input->get('sales_agent')) || !empty($this->input->get('travel_date'))) { ?>
        $('#city_sale_header').click();
    <?php } ?>

    $('#reset').click(function() {
        Reset('<?php echo base_url('Report/City_Sales'); ?>');
    });
</script>