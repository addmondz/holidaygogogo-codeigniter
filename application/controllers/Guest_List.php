...
class Guest_List extends CI_Controller
{
	...
	function Download() {
		...
		$spreadsheet->getActiveSheet()->setCellValue('D1', 'CUSTOMER MOBILE');
		$spreadsheet->getActiveSheet()->setCellValue('E1', 'CHAT LANGUAGE');
		$spreadsheet->getActiveSheet()->setCellValue('F1', 'TRAVEL DATE');
		$spreadsheet->getActiveSheet()->setCellValue('G1', 'DESTINATION');
		$spreadsheet->getActiveSheet()->setCellValue('H1', 'SALES AGENT');
		$spreadsheet->getActiveSheet()->setCellValue('I1', 'GUEST TYPE');
		$spreadsheet->getActiveSheet()->setCellValue('J1', 'GUEST');
		$spreadsheet->getActiveSheet()->setCellValue('K1', 'GENDER');
		$spreadsheet->getActiveSheet()->setCellValue('L1', 'DATE OF BIRTH');
		$spreadsheet->getActiveSheet()->setCellValue('M1', 'NATIONALITY');
		$spreadsheet->getActiveSheet()->setCellValue('N1', 'GUEST IDENTIFICATION NUMBER');
		$spreadsheet->getActiveSheet()->setCellValue('O1', 'PASSPORT NUMBER');
		$spreadsheet->getActiveSheet()->setCellValue('P1', 'GUEST MOBILE');
		$spreadsheet->getActiveSheet()->setCellValue('Q1', 'EMAIL');
		$spreadsheet->getActiveSheet()->setCellValue('R1', 'MARITAL STATUS');
		$spreadsheet->getActiveSheet()->setCellValue('S1', 'EMPLOYMENT');
		$spreadsheet->getActiveSheet()->setCellValue('T1', 'ADDRESS');
		$spreadsheet->getActiveSheet()->setCellValue('U1', 'POSTCODE');
		$spreadsheet->getActiveSheet()->setCellValue('V1', 'CITY');
		$spreadsheet->getActiveSheet()->setCellValue('W1', 'STATE');
		$spreadsheet->getActiveSheet()->setCellValue('X1', 'COUNTRY');
		$spreadsheet->getActiveSheet()->setCellValue('Y1', 'NOMINEE');
		$spreadsheet->getActiveSheet()->setCellValue('Z1', 'NOMINEE IDENTIFICATION NUMBER');
		$spreadsheet->getActiveSheet()->setCellValue('AA1', 'RELATIONSHIP');
		$row = 2;
		$guest_lists = $this->Guest_List_Model->Read_Guest_Lists1();
		$spreadsheet->getActiveSheet()->getStyle('A1:AA1')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB(\PhpOffice\PhpSpreadsheet\Style\Color::COLOR_BLACK);
		...
		foreach($guest_lists as $guest) {
			...
			$spreadsheet->getActiveSheet()->setCellValueExplicit('C' . $row, $guest->Customer, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('D' . $row, $guest->CustomerMobile, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('E' . $row, $guest->ChatLanguage, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('F' . $row, $guest->TravelDate, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('G' . $row, $guest->Destination, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('H' . $row, $guest->SalesAgent, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('I' . $row, $guest->Type, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('J' . $row, $guest->Guest, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('K' . $row, $guest->Gender, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('L' . $row, $guest->DateOfBirth, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('M' . $row, $guest->Nationality, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('N' . $row, $guest->IdentificationNumber, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('O' . $row, $guest->PassportNumber, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('P' . $row, $guest->GuestMobile, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('Q' . $row, $guest->Email, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('R' . $row, $guest->MaritalStatus, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('S' . $row, $guest->Employment, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('T' . $row, $guest->Address, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('U' . $row, $guest->Postcode, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('V' . $row, $guest->City, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('W' . $row, $guest->State, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('X' . $row, $guest->Country, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('Y' . $row, $guest->Nominee, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('Z' . $row, $guest->NomineeIdentificationNumber, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$spreadsheet->getActiveSheet()->setCellValueExplicit('AA' . $row, $guest->Relationship, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
			$row++;
		}
		$spreadsheet->getActiveSheet()->getStyle('A:AA')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT);
