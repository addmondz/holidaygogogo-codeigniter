...
/**
 * Our Product — an Owner-only (level 10) tool. The Owner pastes a
 ...
 */
class Our_Product extends MY_Controller
{
	...
	{
		$titles = array(
			'tab_title'        => 'HolidayGoGoGo | Our Product',
			'breadcrumb_title' => 'Our Product',
		);
		$this->load->view('layout/header', $titles);
		$this->load->view('our_product/index');
	...
	function Review()
	{
		...
		$titles = array(
			'tab_title'        => 'HolidayGoGoGo | Our Product',
			'breadcrumb_title' => 'Our Product >> Review',
		);
		$this->load->view('layout/header', $titles);
		$this->load->view('our_product/review', array(
			'job'      => $job_id,
	...
	function Delete_Crawl_Items()
	{
		...
		foreach ($result['deleted_analysis_ids'] as $aid) {
			$this->Our_Product_Model->Delete((int) $aid);
		}
	...
	function Job_State()
	{
		...
		$this->output->set_content_type('application/json');
		$job_id = preg_replace('/[^A-Za-z0-9_]/', '', (string) $this->input->get('job'));
		$s = json_decode((string) @file_get_contents(APPPATH . 'logs/our_product_crawl/jobs/' . $job_id . '.json'), true);
	...
	function Analyze()
	{
		...
		if ( ! $has_file && $paste !== '') {
			...
			try {
				...
				$this->Our_Product_Model->Create(array(
					'url'           => competitor_paste_source_label($paste),
					'source'        => 'paste',
					'status'        => 'error',
			...
			$id = $this->Our_Product_Model->Create($record);
		...
		// Our Product analyses OUR OWN catalogue only — reject any URL that is not
		// on the holidaygogogo.com domain (mirrors the client-side holidayUrl gate).
		if ( ! $has_file) {
			$host = strtolower((string) parse_url($url, PHP_URL_HOST));
			if ($host !== 'www.holidaygogogo.com' && $host !== 'holidaygogogo.com') {
				echo json_encode(array('success' => false, 'message' => 'Only https://www.holidaygogogo.com/ URLs are allowed here.'));
				return;
			}
		}

		// URL → background single-page read job (non-blocking; polled in the history
		// table). The user pastes the exact tour detail page, so there is no
		// discovery — the worker deep-reads just that page.
		if ( ! $has_file) {
			$job_id = $this->start_crawl_job($url);
			if ($job_id !== '') {
				echo json_encode(array('success' => true, 'job' => $job_id));
			} else {
				echo json_encode(array('success' => false, 'message' => 'Could not start the background read (process spawn unavailable).'));
			}
			return;
		}
	...
	/**
	 * AJAX: the Analysis Results table (newest first). Every run is a distinct
	 * pasted tour detail URL, so crawls are NOT merged by host — each read shows as
	 * its own row (no Timeline grouping) with its own Review & Select / Terminate /
	 * Delete. Pasted text/links and uploaded PDFs/images stay as their own rows too.
	 * Prunes stale job files. Returns {jobs: [...], running: <bool>}.
	 */
	function Jobs_List()
	{
		if ( ! $this->input->is_ajax_request()) {
		...
		// Each crawl run is its own row (single pasted tour URL — nothing to group).
	 ...
	 * worker (Our_Product_Job::run). Returns the job id, or '' when exec is
	...
	private function queue_job($status)
	{
		...
		// Controller name MUST match the file case exactly (Our_Product_Job) — Linux
		...
		$cmd = escapeshellarg($php) . ' -d pcre.jit=0 -d memory_limit=768M ' . escapeshellarg($index) . ' Our_Product_Job run ' . escapeshellarg($job_id)
	...
	{
		$dir = FCPATH . 'assets/upload/our_product/';
		if ( ! is_dir($dir)) {
			@mkdir($dir, 0755, true);
		}
		$config = array(
	...
	function View()
	{
		...
		$analysis = $this->Our_Product_Model->Read_One($id);
		if ( ! $analysis) {
			redirect(base_url('Our_Product'));
			return;
		}
		$lang   = competitor_normalize_lang($this->input->get('lang'));
		$has_cn = $this->Our_Product_Model->Read_Translation($id, 'cn') !== null;
		if ($lang !== 'en') {
			$analysis = $this->apply_language($analysis, $id, $lang);
		}
		$titles = array(
			'tab_title'        => 'HolidayGoGoGo | Our Product',
			'breadcrumb_title' => 'Our Product >> View',
		);
		$this->load->view('layout/header', $titles);
		$this->load->view('our_product/view', array(
			'a'      => $analysis,
			'lang'   => $lang,
	...
	function Translate()
	{
		...
		if ($this->Our_Product_Model->Read_Translation($id, $lang) !== null) {
		...
		$this->Our_Product_Model->Save_Translation($id, $lang, $overlay);
	...
	{
		$tr = $this->Our_Product_Model->Read_Translation($id, $lang);
		if (is_array($tr)) {
	 ...
	 * out for DomPDF via the our_product/pdf template). Owner-only via the
	 * constructor gate.
	 */
	function Download_Pdf()
	{
		...
		$html = $this->load->view('our_product/pdf', array(
			'a'        => $analysis,
			'lang'     => $lang,
			'labels'   => competitor_ui_labels($lang),
	...
	function Delete()
	{
		...
		$this->output->set_content_type('application/json');
		$ok = $this->Our_Product_Model->Delete((int) $this->input->post('id'));
