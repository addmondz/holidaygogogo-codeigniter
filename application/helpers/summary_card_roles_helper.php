 ...
 * set on each:
 *
 *   - Booking listing  (`$owner_as_agent = true`):  Owner (10) and TC Lead (25)
 ...
 *     per-agent performance matrix instead.
 *
 * TC Lead (25) always shows the sales-agent set (it only ever loads the partial
 * from the booking listing), replacing its former team-lead-specific cards.
 *
...
if (!function_exists('summary_cards_show_agent_set')) {
    ...
    {
        $level = (int) $level;
        // Sales Agent (20), TC (50) and TC Lead (25) always get the agent cards.
        if ($level === 20 || $level === 50 || $level === 25) {
            return true;
        }
        // Owner (10) gets them only on the booking listing.
        return ($level === 10 && (bool) $owner_as_agent);
    }
}
